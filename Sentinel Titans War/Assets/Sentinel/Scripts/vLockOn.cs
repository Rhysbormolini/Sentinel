...
namespace Gamekit3D
{
    public class vLockOn : vLockOnBehaviour
    {
        ...
        {
            Debug.Log("is tp camera null: " + tpCamera);
            if (tpCamera == null) return;
            

            // lock the camera into a target, if there is any around
            if (Input.GetKeyDown(KeyCode.Tab) || Input.GetButtonDown("Target"))
            {
                Debug.Log("Should be locking on");
                isLockingOn = !isLockingOn;
                LockOn(isLockingOn);
            }
            // unlock the camera if the target is null
            else if (isLockingOn && (tpCamera.lookAt == null))
            {
                isLockingOn = false;
                LockOn(false);
            }
        ...
        protected override void SetTarget()
        {
            if (tpCamera != null)
            {
                //tpCamera.SetLockTarget(currentTarget.transform, cameraHeightOffset, lockSpeed);
        ...
        protected virtual void SwitchTargetsInput()
        {
            ...
            if (tpCamera.lookAt)
            {
                // TODO: Map for Joystick controls
        ...
        protected virtual void UpdateAimImage()
        {
            ...
            if (currentTarget && aimImage && aimImageContainer)
                aimImage.anchoredPosition = currentTarget.GetScreenPointOffBoundsCenter(aimImageContainer, playerCam, spriteHeight);
        ...
        {
            if (currentTarget == null && tpCamera != null)
            {
                onUnLockOnTarget.Invoke(tpCamera.lookAt);
                tpCamera.lookAt = transform;
                isLockingOn = false;
                inTarget = false;
            }
        }
